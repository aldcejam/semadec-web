...
export const SuapClient = ({
  ...
  const extractToken = () => {
    ...
    let match = typeof window !== "undefined" ? document.location.hash.match(/access_token=(\w+)/) : null;
    if (match != null) {
  ...
  let extractScope = function () {
    let match = typeof window !== "undefined" ? document.location.hash.match(/scope=(.*)/) : null;
    if (match != null) {
  ...
  let extractDuration = function () {
    let match = typeof window !== "undefined" ? document.location.hash.match(/expires_in=(\d+)/) : null;
